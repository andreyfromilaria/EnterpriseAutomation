...
/************************************************************************************************************/
CExcel::CExcel() {}
CExcel::CExcel(const char* filename)
{
	Load(filename);
}
...
void CExcel::New(int sheets)
{
	workbook_ = Workbook();
	worksheets_.clear();

	workbook_.fonts_.resize(1); //MF was 4, see CExcelFormatManager::get_font_idx()
...
bool CExcel::Load(const char* filename)
{
	if (file_.IsOpen())
		file_.Close();
...
bool CExcel::Load(const wchar_t* filename)
{
	if (file_.IsOpen())
		file_.Close();
...
bool CExcel::SaveAs(const char* filename)
{
	if (file_.IsOpen())
		file_.Close();
...
bool CExcel::SaveAs(const wchar_t* filename)
{
	if (file_.IsOpen())
		file_.Close();
...
CExcelWorksheet* CExcel::GetWorksheet(int sheetIndex)
{
...
CExcelWorksheet* CExcel::GetWorksheet(const char* name)
{
...
// Returns 0 if there is no Excel worksheet with the given name.
CExcelWorksheet* CExcel::GetWorksheet(const wchar_t* name)
{
	int maxWorksheets = (int) yesheets_.size();
...
CExcelWorksheet* CExcel::AddWorksheet(int sheetIndex)
{
	int sheetNo = (int)yesheets_.size() + 1;
	CExcelWorksheet* yesheet = 0;
...
// Worksheet is added to the last position if sheetIndex == -1.
// Returns a pointer to the worksheet if successful, 0 if otherwise.
CExcelWorksheet* CExcel::AddWorksheet(const char* name, int sheetIndex)
{
	int maxWorksheets = (int) yesheets_.size();
	...
	Worksheet* worksheet;
	CExcelWorksheet* yesheet;

	if (sheetIndex == -1) {
		workbook_.boundSheets_.push_back(Workbook::BoundSheet());
		worksheets_.push_back(Worksheet());
		yesheets_.push_back(new CExcelWorksheet(this, (int)worksheets_.size()-1));
		...
		yesheet = &**(yesheets_.insert(yesheets_.begin()+sheetIndex, new CExcelWorksheet(this, sheetIndex)));
...
// Worksheet is added to the last position if sheetIndex == -1.
// Returns a pointer to the worksheet if successful, 0 if otherwise.
CExcelWorksheet* CExcel::AddWorksheet(const wchar_t* name, int sheetIndex)
{
	int maxWorksheets = (int)yesheets_.size();
...
bool CExcel::DeleteWorksheet(int sheetIndex)
{
...
// Returns true if successful, false if otherwise.
bool CExcel::DeleteWorksheet(const char* name)
{
	int maxWorksheets = (int)yesheets_.size();
...
// Returns true if successful, false if otherwise.
bool CExcel::DeleteWorksheet(const wchar_t* name)
{
...
char* CExcel::GetAnsiSheetName(int sheetIndex)
{
...
wchar_t* CExcel::GetUnicodeSheetName(int sheetIndex)
{
	if (workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1)
...
bool CExcel::GetSheetName(int sheetIndex, char* name)
{
	if (!(workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1))
	{
...
bool CExcel::GetSheetName(int sheetIndex, wchar_t* name)
{
	if (workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1)
	{
...
// Index starts from 0.
// Returns true if successful, false if otherwise.
bool CExcel::RenameWorksheet(int sheetIndex, const char* to)
{
	int maxWorksheets = (int)yesheets_.size();
...
// Index starts from 0.
// Returns true if successful, false if otherwise.
bool CExcel::RenameWorksheet(int sheetIndex, const wchar_t* to)
{
	int maxWorksheets = (int)yesheets_.size();
	if (sheetIndex < maxWorksheets) {
		for(int i=0; i<maxWorksheets; ++i) {
...
// Returns true if successful, false if otherwise.
bool CExcel::RenameWorksheet(const char* from, const char* to)
{
	int maxWorksheets = (int)yesheets_.size();
	for(int i=0; i<maxWorksheets; ++i) {
		if (workbook_.boundSheets_[i].name_.unicode_ & 1)
...
// Returns true if successful, false if otherwise.
bool CExcel::RenameWorksheet(const wchar_t* from, const wchar_t* to)
{
	int maxWorksheets = (int) worksheets_.size();
...
size_t CExcel::Read(const char* data, size_t dataSize)
{
	size_t bytesRead = 0;
...
void CExcel::AdjustStreamPositions()
{
...
void CExcel::AdjustBoundSheetBOFPositions()
{
...
void CExcel::AdjustDBCellPositions()
{
...
void CExcel::AdjustExtSSTPositions()
{
...
void CExcel::UpdateYExcelWorksheet()
{
	int maxWorksheets = (int) worksheets_.size();
	yesheets_.clear();
	...
	for(int i=0; i<maxWorksheets; ++i) {
		yesheets_.push_back(new CExcelWorksheet(this, i));
...
void CExcel::UpdateWorksheets()
{
	...
	for(int s=0; s<maxWorksheets; ++s) {
		const CExcelWorksheet& sheet = *yesheets_[s];
		...
		for(int r=0, curRowBlock=0; r<maxRows; ++r) {
			...
			for(int c=0; c<maxCols; ++c) {
				const CExcelCell* cell = sheet.Cell(r, c);

				int cellType = cell->Type();
//				if (cellType != CExcelCell::UNDEFINED)	// Current cell contains some data
				// Write cell content, even if blank in order to keep format
				{
					...
					switch(cellType) {
						case CExcelCell::INT:
						{
							...
							int cl = c + 1;
							for(; cl<maxCols; ++cl) {
								const CExcelCell* cellNext = sheet.Cell(r, cl);
								if (cellNext->Type()==CExcelCell::UNDEFINED ||
		...
		for(int mr=0; mr<maxRows; ++mr) {
			for(int c=0; c<maxCols; ++c) {
				const CExcelCell* cell = sheet.Cell(mr,c);
...
/************************************************************************************************************/
CExcelWorksheet::CExcelWorksheet(CExcel* excel, int sheetIndex) :
...
// Returns 0 if name is in Unicode format.
char* CExcelWorksheet::GetAnsiSheetName()
{
...
// Get the current worksheet name.
// Returns 0 if name is in Ansi format.
wchar_t* CExcelWorksheet::GetUnicodeSheetName()
{
	if (excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1)
...
// Get the current worksheet name.
// Returns false if name is in Unicode format.
bool CExcelWorksheet::GetSheetName(char* name)
{
	if (!(excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1))
	{
...
// Get the current worksheet name.
// Returns false if name is in Ansi format.
bool CExcelWorksheet::GetSheetName(wchar_t* name)
{
	if (excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1)
	{
...
// Returns true if successful, false if otherwise.
bool CExcelWorksheet::Rename(const char* to)
{
...
// Returns true if successful, false if otherwise.
bool CExcelWorksheet::Rename(const wchar_t* to)
{
	int maxWorksheets = (int) excel_->workbook_.boundSheets_.size();
	for(int i=0; i<maxWorksheets; ++i) {
		if (!(excel_->workbook_.boundSheets_[i].name_.unicode_ & 1)) continue;
...
void CExcelWorksheet::Print(ostream& os, char delimiter, char textQualifier) const
{
	for(int r=0; r<maxRows_; ++r)
	{
		for(int c=0; c<maxCols_; ++c)
		{
			const CExcelCell* cell = Cell(r, c);
			switch(cell->Type())
			{
				case CExcelCell::UNDEFINED:
					break;
...
int CExcelWorksheet::GetTotalRows() const
{
	return maxRows_;
}

// Total number of columns in current Excel worksheet.
int CExcelWorksheet::GetTotalCols() const
{
	return maxCols_;
}
...
CExcelCell* CExcelWorksheet::Cell(int row, int col)
{
	...
	if (row >= maxRows_) {
		...
		maxRows_ = row + 1;
		cells_.resize(maxRows_, vector<CExcelCell>(maxCols_));
	}
...
// row and col starts from 0.
// Returns NULL if row exceeds 65535 or col exceeds 255.
const CExcelCell* CExcelWorksheet::Cell(int row, int col) const
{
...
bool CExcelWorksheet::EraseCell(int row, int col)
{
...
void CExcelWorksheet::SetColWidth(const int colindex, const USHORT colwidth)
{
...
USHORT CExcelWorksheet::GetColWidth(const int colindex)
{
...
// Update cells using information from CExcel.worksheets_
void CExcelWorksheet::UpdateCells()
{
...
//	maxCols_ = dimension.lastUsedColIndexPlusOne_;
	...
	vector<CExcelCell> cellCol(maxCols_);
...
void CExcelWorksheet::MergeCells(int row, int col, USHORT rowRange, USHORT colRange)
{
	CExcelCell* cell = Cell(row, col);

	cell->SetMergedRows(rowRange);
	cell->SetMergedColumns(colRange);

/* Assembling MERGECELL records is accomplished in CExcel::UpdateWorksheets()
	Worksheet::MergedCells::MergedCell mergedCell;
...
int CExcelCell::Type() const {return type_;}

// Get an integer value.
// Returns false if cell does not contain an integer or a double.
bool CExcelCell::Get(int& val) const
{
	if (type_ == INT) {
		val = ival_;
		return true;
...
bool CExcelCell::Get(double& val) const
{
	if (type_ == DOUBLE) {
		val = dval_;
		return true;
...
bool CExcelCell::Get(char* str) const
{
	if (type_ == STRING) {
		if (str_.empty()) *str = '\0';
...
bool CExcelCell::Get(wchar_t* str) const
{
	if (type_ == WSTRING) {
		if (wstr_.empty())
			*str = L'\0';
...
size_t CExcelCell::GetStringLength() const
{
	if (type_ == STRING)
		return str_.size() - 1;
	else {
...
int CExcelCell::GetInteger() const
{
	int val;
...
double CExcelCell::GetDouble() const
{
	double val;
...
const char* CExcelCell::GetString() const
{
...
const wchar_t* CExcelCell::GetWString() const
{
...
void CExcelCell::Set(int val)
{
	SetInteger(val);
}

// Set content of current Excel cell to a double.
void CExcelCell::Set(double val)
{
	SetDouble(val);
}

// Set content of current Excel cell to an ANSI string.
void CExcelCell::Set(const char* str)
{
	SetString(str);
}

// Set content of current Excel cell to an Unicode string.
void CExcelCell::Set(const wchar_t* str)
{
	SetWString(str);
}

// Set content of current Excel cell to an integer.
void CExcelCell::SetInteger(int val)
{
	type_ = INT;
	ival_ = val;
}
...
// Set content of current Excel cell to a double.
void CExcelCell::SetDouble(double val)
{
	type_ = DOUBLE;
	dval_ = val;
}
...
void CExcelCell::SetRKValue(int rkValue)
{
...
// Set content of current Excel cell to an ANSI string.
void CExcelCell::SetString(const char* str)
{
...
// Set content of current Excel cell to an Unicode string.
void CExcelCell::SetWString(const wchar_t* str)
{
...
//MF
void CExcelCell::SetFormula(const Worksheet::CellTable::RowBlock::CellBlock::Formula& f)
{
	type_ = CExcelCell::FORMULA;
	_pFormula = new Formula(f);
}

CExcelCell::Formula::Formula(const Worksheet::CellTable::RowBlock::CellBlock::Formula& f)
 :	  _formula_type(0)
{
...
bool CExcelCell::get_formula(Worksheet::CellTable::RowBlock::CellBlock* pCell) const
{
...
ostream& operator<<(ostream& os, const CExcelCell& cell)
{
	switch(cell.Type())
	{
		case CExcelCell::UNDEFINED:
