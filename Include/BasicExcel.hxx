...
public: // Worksheet functions.
	...
	CExcelWorksheet* GetWorksheet(int sheetIndex);	///< Get a pointer to an Excel worksheet at the given index. Index starts from 0. Returns 0 if index is invalid.
	CExcelWorksheet* GetWorksheet(const char* name);	///< Get a pointer to an Excel worksheet that has given ANSI name. Returns 0 if there is no Excel worksheet with the given name.
	CExcelWorksheet* GetWorksheet(const wchar_t* name); ///< Get a pointer to an Excel worksheet that has given Unicode name. Returns 0 if there is no Excel worksheet with the given name.

	CExcelWorksheet* AddWorksheet(int sheetIndex=-1);	///< Add a new Excel worksheet to the given index. Name given to worksheet is SheetX, where X is a number which starts from 1. Index starts from 0. Worksheet is added to the last position if sheetIndex == -1. Returns a pointer to the worksheet if successful, 0 if otherwise.
	CExcelWorksheet* AddWorksheet(const char* name, int sheetIndex=-1); ///< Add a new Excel worksheet with given ANSI name to the given index. Index starts from 0. Worksheet is added to the last position if sheetIndex == -1. Returns a pointer to the worksheet if successful, 0 if otherwise.
	CExcelWorksheet* AddWorksheet(const wchar_t* name, int sheetIndex=-1);	///< Add a new Excel worksheet with given Unicode name to the given index. Index starts from 0. Worksheet is added to the last position if sheetIndex == -1. Returns a pointer to the worksheet if successful, 0 if otherwise.
...
public:
	...
	vector<SmartPtr<CExcelWorksheet> > yesheets_;	///< Parsed Worksheets.
};

class CExcelWorksheet : public RefCount::RefCnt
{
	friend class CExcel;

public:
	CExcelWorksheet(CExcel* excel, int sheetIndex);
...
public: // Cell functions
	...
	CExcelCell* Cell(int row, int col); ///< Return a pointer to an Excel cell. row and col starts from 0. Returns 0 if row exceeds 65535 or col exceeds 255.
	const CExcelCell* Cell(int row, int col) const; ///< Return a pointer to an Excel cell. row and col starts from 0. Returns 0 if row exceeds 65535 or col exceeds 255.
...
private: // Internal functions
	void UpdateCells(); ///< Update cells using information from CExcel.worksheets_.

private:
	CExcel* excel_; 				///< Pointer to instance of CExcel.
	...
	vector<vector<CExcelCell> > cells_; ///< Cells matrix.
...
public:
	...
	friend ostream& operator<<(ostream& os, const CExcelCell& cell);	///< Print cell to output stream. Print a null character if cell is undefined.
	...
	void SetFormat(const Ilaria::TCellFormat& fmt); //MF
...
private:
	...
	friend class CExcel;
	friend class CExcelWorksheet; // for Get(w/char*);
