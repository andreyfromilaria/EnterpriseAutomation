...
enum EXCEL_FONT_OPTIONS {
	EXCEL_FONT_BOLD			= 0x01,	// redundant in BIFF5-BIFF8, see TExcelFont::_weight
	EXCEL_FONT_ITALIC		= 0x02,
	EXCEL_FONT_UNDERLINED	= 0x04,	// redundant in BIFF5-BIFF8, see TExcelFont::_underline_type
...
struct TExcelFont
{
	...
	TExcelFont(const Workbook::Font& font)
	...
	TExcelFont& SetHeight(int height)
	{
		_height = height;
		return *this;
	}

	TExcelFont& SetWeight(int weight)
	{
		_weight = weight;
		return *this;
	}

	TExcelFont& SetItalic(bool italic)
	{
		_options = (_options&~EXCEL_FONT_ITALIC) | (italic? EXCEL_FONT_ITALIC: 0);
		return *this;
	}

	TExcelFont& SetColorIndex(int color_idx)
	{
		_color_index = color_idx;
		return *this;
	}

	TExcelFont& SetUnderlineType(char underline_type)
	{
		_underline_type = underline_type;
		return *this;
	}

	TExcelFont& SetEscapementType(short escapement_type)
	{
		_escapement_type = escapement_type;
		return *this;
	}

	TExcelFont& SetFontName(const wchar_t* name)
	{
		_name = name;
		return *this;
	}
...
class CExcelFormatManager
{
public:
	CExcelFormatManager(CExcel& xls);

	const Workbook::Font& GetFont(const TCellFormat& fmt) const;
	wstring GetFormatString(const TCellFormat& fmt) const;
	const Workbook::XF& GetXF(int xf_idx) const {return _xls.workbook_.XFs_[xf_idx];}

	int GetFontIndex(const TExcelFont& font);
	int GetFormatIndex(const wstring& fmt_str);
	int GetXFIndex(const TCellFormat& fmt);
...
struct TCellFormat
{
	TCellFormat(CExcelFormatManager& mgr)
	 :	_mgr(mgr)
	{
		init();
	}

	TCellFormat(CExcelFormatManager& mgr, const TExcelFont& font)
	 :	_mgr(mgr)
	{
		init();
		SetFont(font);
	...
	TCellFormat(CExcelFormatManager& mgr, const Workbook::XF& xf, int xf_idx)
	 :	_mgr(mgr)
	{
		init(xf, xf_idx);
	}

	TCellFormat(CExcelFormatManager& mgr, const CExcelCell* cell)
	 :	_mgr(mgr)
	{
		int xf_idx = cell->GetXFormatIdx();
		init(mgr.GetXF(xf_idx), xf_idx);
	}

	CExcelFormatManager& GetFormatManager() const
	{
		return _mgr;
	}
	...
	TExcelFont GetFont()
	{
		return TExcelFont(_mgr.GetFont(*this));
	}
	TCellFormat& SetFont(const TExcelFont& font)
	{
		_font_idx = _mgr.GetFontIndex(font);
		Flush();
		return *this;
	}

	wstring GetFormatString() const
	{
		return _mgr.GetFormatString(*this);
	}
	TCellFormat& SetFormatString(const char* fmt_str)
	{
		return SetFormatString(widen_string(fmt_str));
	}
	TCellFormat& SetFormatString(const wstring& fmt_str)
	{
		_fmt_idx = _mgr.GetFormatIndex(fmt_str);
		Flush();
		return *this;
	}

	int GetColorFirst() const
	{
		return _color1;
	}
	TCellFormat& SetColorFirst(unsigned color)
	{
		_color1 = color;
		Flush();
		return *this;
	...
	unsigned short GetColorSecond() const
	{
		return _color2;
	}
	TCellFormat& SetColorSecond(unsigned short color)
	{
		_color2 = color;
		Flush();
		return *this;
	...
	TCellFormat& SetBackground(unsigned short color2, unsigned color1=COLOR1_PAT_SOLID)
	{
		return SetColorFirst(color1).SetColorSecond(color2);
	}
	...
	TCellFormat& SetAlignment(char alignment)
	{
	...
	TCellFormat& SetWrapping(bool wrap)
	{
		return SetAlignment((_alignment&~0x08) | (wrap? 0x08: 0x00));
	}
	...
	TCellFormat& SetRotartion(char rotation)
	{
		_rotation = rotation;
		Flush();
		return *this;
	...
	char GetTextProps() const
	{
		return _text_props;
	}
	TCellFormat& SetTextProps(char text_props)
	{
	...
	int GetBorderlines() const
	{
		return _borderlines;
	}
	TCellFormat& SetBorderlines(int borderlines)
	{
	...
	void GetXF(Workbook::XF& xf) const
	{
...
inline void CExcelCell::SetFormat(const Ilaria::TCellFormat& fmt)
{
	_xf_idx = fmt.GetXFIndex();
}
